...
async function getTouristDurationsConfig(): Promise<Record<string, number> | null> {
  try {
    const rows = await db.select().from(settings as any).where(eq(settings.key as any, 'permit.tourist_durations')).limit(1);
    if (rows && rows.length > 0) {
...
router.post('/:id/renew', isAuthenticated, async (req, res) => {
  try {
    ...
    // Vérifier que la date d'expiration est atteinte
    ...
    if (expiryDate) {
      ...
      if (expiryDateObj > today) {
        return res.status(400).json({
          message: "Le renouvellement n'est possible qu'après la date d'expiration du permis.",
          expiryDate: expiryDate
        });
      }
    }

    // Vérifier le nombre maximal de renouvellements
    let metadataCheck: any = row?.metadata;
    try {
      if (typeof metadataCheck === 'string') metadataCheck = JSON.parse(metadataCheck);
    } catch {}
    const currentRenewals = Array.isArray(metadataCheck?.renewals) ? metadataCheck.renewals.length : 0;
    if (currentRenewals >= 2) {
      return res.status(400).json({
        message: "Nombre maximal de renouvellements atteint (maximum 2 renouvellements).",
