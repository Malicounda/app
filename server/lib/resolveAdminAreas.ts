...
export async function resolveAdministrativeAreas(lat: number, lon: number): Promise<AdminAreas> {
  ...
  if (DEBUG) console.debug(`[resolveAdministrativeAreas] Recherche arrondissement pour lat=${lat}, lon=${lon}`);
  ...
  if (DEBUG) console.debug(`[resolveAdministrativeAreas] Arrondissements trouvés:`, arrRows);
  ...
  if (arrRows && arrRows[0]) {
    ...
    if (DEBUG) console.debug(`[resolveAdministrativeAreas] Arrondissement trouvé: ${arrondissement}, departement_id: ${departementId}`);
  } else {
    if (DEBUG) console.debug(`[resolveAdministrativeAreas] Aucun arrondissement trouvé même avec zone tampon`);
  }
