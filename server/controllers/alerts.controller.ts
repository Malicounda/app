...
export const createAlert = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let alertLocalite: string | null = null;
        try {
            if (lat !== null && lon !== null) {
                ...
                alertLocalite = areas.localite ?? null;
        ...
        const responseAlert: AlertResponse = {
            ...
            commune: newAlert.commune ?? alertCommune ?? null,
            arrondissement: newAlert.arrondissement ?? alertArrondissement ?? null,
            localite: alertLocalite,
    ...
    try {
        ...
        const responseNotifications: NotificationResponse[] = await Promise.all(validNotifications.map(async (notif) => {
            ...
            if (notif.alert) {
                ...
                let computedLocalite: string | null = null;
                ...
                if (latNum != null && lonNum != null) {
                    try {
                        ...
                        computedLocalite = areas.localite ?? null;
                    } catch (e) {
        ...
        const responseAlerts: AlertResponse[] = await Promise.all(alertsWithUsers.map(async (alert) => {
            ...
            if (latNum != null && lonNum != null) {
                try {
                    ...
                    computedLocalite = resolved.localite;
                } catch (e) {
