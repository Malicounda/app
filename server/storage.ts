...
import {
    ...
    permitRequests,
    permits,
    reforestationFieldData,
    reforestationLocalites,
    reforestationNurseryTypes,
    reforestationPlantsData,
    reforestationProductionData,
    reforestationReports,
    reforestationSpeciesCatalog,
    reforestationSpeciesCategories,
    reforestationSpeciesData,
    superAdmins,
    taxes,
    userDomains,
    users,
    ...
    type InsertHunter,
    type NewHuntingGuide as InsertHuntingGuide,
    type InsertHuntingReport,
    type InsertMessage,
    type InsertPermit,
    type InsertPermitRequest,
    type InsertReforestationFieldData,
    type InsertReforestationLocalite,
    type InsertReforestationPlantsData,
    type InsertReforestationProductionData,
    type InsertReforestationReport,
    type InsertReforestationSpeciesData,
    type InsertTax,
    type InsertUser,
    type InsertUserDomain,
    type Message,
    type MessageWithSender,
    type Permit,
    type PermitRequest,
    type ReforestationLocalite,
    type ReforestationNurseryType,
...
import { db } from "./db.js";
  ...
  export interface IStorage {
    ...
    getUserDomains(userId: number): Promise<number[]>;
    getUserDomainsByUserId(userId: number): Promise<UserDomain[]>;
    getUserDomainById(id: number): Promise<UserDomain | undefined>;
    createUserDomain(data: InsertUserDomain): Promise<UserDomain>;
    updateUserDomain(id: number, data: Partial<InsertUserDomain>): Promise<UserDomain | undefined>;
    deleteUserDomain(id: number): Promise<boolean>;
    isSuperAdmin(userId: number): Promise<boolean>;
    getSuperAdminUserIds(): Promise<number[]>;
    getAllDomains(): Promise<(typeof domaines.$inferSelect)[]>;

    // Reforestation (CNR) methods
    getPepinieresForMap(): Promise<any[]>;
    getReforestationZonesForMap(): Promise<any[]>;
    getRegionalReforestationStats(region?: string): Promise<any>;
    getMySectorAgentsByDomain(region: string, domain: string): Promise<User[]>;
    getReforestationActivities(region?: string): Promise<any[]>;
    checkReforestationReportExists(userId: number, period: string): Promise<boolean>;
    createReforestationReport(report: InsertReforestationReport, production: InsertReforestationProductionData[], plants: InsertReforestationPlantsData[], species: InsertReforestationSpeciesData[], field: InsertReforestationFieldData[]): Promise<ReforestationReport>;
    getReforestationReports(filters: any): Promise<ReforestationReport[]>;
    getConsolidatedNationalReforestationData(period: string): Promise<any>;
    getConsolidatedReforestationData(region: string, period: string): Promise<any>;
    getReforestationReportById(id: number): Promise<any>;
    updateReforestationReportStatus(id: number, status: string): Promise<ReforestationReport | undefined>;
    deleteReforestationReport(id: number): Promise<boolean>;
    getLastReforestationReport(location: { region: string; departement?: string; arrondissement?: string; commune?: string }): Promise<ReforestationReport | undefined>;
    getReforestationLocalites(departement: string): Promise<ReforestationLocalite[]>;
    addReforestationLocalite(data: InsertReforestationLocalite): Promise<ReforestationLocalite>;
    softDeleteReforestationLocalite(id: number, userId: number): Promise<boolean>;
    getCatalogSpecies(): Promise<any[]>;
    addCatalogSpecies(name: string, category: string): Promise<any>;
    bulkAddCatalogSpecies(items: any[]): Promise<number>;
    deleteCatalogSpecies(id: number): Promise<boolean>;
    getCatalogCategories(): Promise<any[]>;
    addCatalogCategory(name: string): Promise<any>;
    updateCatalogCategory(id: number, name: string): Promise<any>;
    deleteCatalogCategory(id: number): Promise<boolean>;
    getNurseryTypes(): Promise<ReforestationNurseryType[]>;
  ...
  export class DatabaseStorage implements IStorage {
      ...
      const sentMessages = await db.select().from(messages)
        .where(
          and(
            ...
            sql`${messages.deletedAtSender} IS NULL`,
    ...
    async getGroupReadsWithUsers(messageId: number): Promise<any[]> {
      return await db.select({
        id: groupMessageReads.id,
        messageId: groupMessageReads.messageId,
        userId: groupMessageReads.userId,
        isRead: groupMessageReads.isRead,
        isDeleted: groupMessageReads.isDeleted,
        readAt: groupMessageReads.readAt,
      ...
      .from(groupMessageReads)
      .innerJoin(users, eq(groupMessageReads.userId, users.id))
      .where(eq(groupMessageReads.messageId, messageId));
    }
    ...
    async isSuperAdmin(userId: number): Promise<boolean> {
      try {
        // On vérifie d'abord la colonne isSuperAdmin dans la table users
        ...
        // Fallback: vérifier la présence dans la table super_admins pour la compatibilité
        const superAdminCheck = await db.select().from(superAdmins).where(eq(superAdmins.userId, userId));
        return superAdminCheck.length > 0;
      } catch (err) {
        console.error('[storage.isSuperAdmin] erreur:', err);
        return false;
      }
    }
    ...
    async getSuperAdminUserIds(): Promise<number[]> {
      try {
        const fromTable = await db.select({ userId: superAdmins.userId }).from(superAdmins);
        const fromFlag = await db.select({ id: users.id }).from(users).where(eq(users.isSuperAdmin, true));

        const ids = new Set([
          ...fromTable.map(r => Number(r.userId)),
          ...fromFlag.map(r => Number(r.id))
        ]);

        return Array.from(ids).filter(id => !isNaN(id));
      } catch (error) {
        console.error("Erreur getSuperAdminUserIds:", error);
        return [];
      }
    }

    // Récupère les domaines associés à un utilisateur (IDs uniquement, pour messaging.service)
    async getUserDomains(userId: number): Promise<number[]> {
      try {
        // Si super-admin, accès à TOUS les domaines
        const isSuper = await this.isSuperAdmin(userId);
        if (isSuper) {
          const all = await this.getAllDomains();
          return all.map(d => d.id);
        }

        const domains = await db.select({ domaineId: userDomains.domaineId })
          .from(userDomains)
          .where(and(eq(userDomains.userId, userId), eq(userDomains.active, true)));

        return domains
          .map(d => d.domaineId)
          .filter((id): id is number => id !== null);
      } catch (error) {
        console.error(`Erreur lors de la récupération des IDs de domaines pour l'utilisateur ${userId}:`, error);
        return [];
      }
    }

    // Récupère les objets domaines associés à un utilisateur
    async getUserDomainsByUserId(userId: number): Promise<UserDomain[]> {
      try {
        const isSuper = await this.isSuperAdmin(userId);
        if (isSuper) {
          const all = await this.getAllDomains();
          return all.map(d => ({
            id: 0, // ID factice pour une affectation virtuelle
            userId,
            domain: d.codeSlug,
            domaineId: d.id,
            active: true,
        ...
        return await db.select().from(userDomains).where(eq(userDomains.userId, userId));
      } catch (error) {
        console.error(`Erreur lors de la récupération des domaines pour l'utilisateur ${userId}:`, error);
        return [];
      }
    }

    // Récupère une affectation de domaine par ID
    async getUserDomainById(id: number): Promise<UserDomain | undefined> {
      try {
        const result = await db.select().from(userDomains).where(eq(userDomains.id, id));
        return result[0];
      } catch (error) {
        console.error(`Erreur lors de la récupération de l'affectation domaine ${id}:`, error);
        return undefined;
      }
    }

    // Crée une nouvelle affectation de domaine
    async createUserDomain(data: InsertUserDomain): Promise<UserDomain> {
      try {
        const result = await db.insert(userDomains).values(data).returning();
        return result[0];
      } catch (error) {
        ...
        if (anyErr?.code === '23505') {
          try {
            const existing = await db
              ...
              .from(userDomains)
              .where(and(
                eq(userDomains.userId, (data as any).userId),
                eq(userDomains.domain, (data as any).domain)
              ));
            if (existing?.[0]) return existing[0];
          } catch (e) {
            // Si la lecture échoue, on retombe sur l'erreur originale
          }
        }
        console.error("Erreur lors de la création de l'affectation domaine:", error);
        throw error;
      }
    }

    // Met à jour une affectation de domaine
    async updateUserDomain(id: number, data: Partial<InsertUserDomain>): Promise<UserDomain | undefined> {
      try {
        const result = await db.update(userDomains).set(data).where(eq(userDomains.id, id)).returning();
        return result[0];
      } catch (error) {
        console.error(`Erreur lors de la mise à jour de l'affectation domaine ${id}:`, error);
        throw error;
      }
    }

    // Supprime une affectation de domaine
    async deleteUserDomain(id: number): Promise<boolean> {
      try {
        const result = await db.delete(userDomains).where(eq(userDomains.id, id)).returning();
        return result.length > 0;
      } catch (error) {
        console.error(`Erreur lors de la suppression de l'affectation domaine ${id}:`, error);
        return false;
      }
    }

    // Récupère tous les domaines enregistrés
    async getAllDomains(): Promise<(typeof domaines.$inferSelect)[]> {
      try {
        return await db.select().from(domaines).where(eq(domaines.isActive, true));
      } catch (error) {
        console.error("Erreur lors de la récupération de tous les domaines:", error);
        return [];
      }
    }

    // --- Reforestation (CNR) Implementation ---

    async getNurseryTypes(): Promise<ReforestationNurseryType[]> {
      try {
        return await db.select().from(reforestationNurseryTypes).where(eq(reforestationNurseryTypes.isActive, true));
      } catch (error) {
        console.error("Erreur lors de la récupération des types de pépinières:", error);
        return [];
      }
    }

    async addNurseryType(label: string, departement: string): Promise<ReforestationNurseryType> {
      const code = label.toUpperCase().replace(/\s+/g, '_');
      const result = await db.insert(reforestationNurseryTypes).values({
        label,
        code,
        departement,
        isActive: true
    ...
    async getPepinieresForMap(): Promise<any[]> {
      try {
        // Cette méthode devrait renvoyer les données consolidées des pépinières
        // Pour l'instant, on se base sur les rapports validés
        const results = await db.select({
          id: reforestationProductionData.id,
          localite: reforestationProductionData.localite,
          nurseryType: reforestationProductionData.nurseryType,
          nbPepinieresPeriode: reforestationProductionData.nbPepinieresPeriode,
          region: reforestationReports.region,
          departement: reforestationReports.departement,
          reportDate: reforestationReports.reportDate
        })
        .from(reforestationProductionData)
        .innerJoin(reforestationReports, eq(reforestationProductionData.reportId, reforestationReports.id))
        .where(and(
          eq(reforestationReports.status, 'valide'),
          sql`${reforestationProductionData.nbPepinieresPeriode} > 0`
        ));

        return results.map(r => ({
          ...r,
          lat: 14.5 + (Math.random() - 0.5) * 2, // Mock coordinates if PostGIS geom not yet used for this
          lon: -14.5 + (Math.random() - 0.5) * 2
        }));
      } catch (error) {
        console.error("Erreur getPepinieresForMap:", error);
        return [];
      }
    }

    async getReforestationZonesForMap(): Promise<any[]> {
      try {
        const results = await db.select({
          id: reforestationFieldData.id,
          localite: reforestationFieldData.localite,
          region: reforestationReports.region,
          departement: reforestationReports.departement,
          pmRegieHa: reforestationFieldData.pmRegieHa,
          pmPriveHa: reforestationFieldData.pmPriveIndivHa,
          reportDate: reforestationReports.reportDate
        })
        .from(reforestationFieldData)
        .innerJoin(reforestationReports, eq(reforestationFieldData.reportId, reforestationReports.id))
        .where(eq(reforestationReports.status, 'valide'));

        return results.map(r => ({
          ...r,
          lat: 14.5 + (Math.random() - 0.5) * 2,
          lon: -14.5 + (Math.random() - 0.5) * 2
        }));
      } catch (error) {
        console.error("Erreur getReforestationZonesForMap:", error);
        return [];
      }
    }

    async getRegionalReforestationStats(region?: string): Promise<any> {
      try {
        const filters = [eq(reforestationReports.status, 'valide')];
        if (region) {
          filters.push(eq(reforestationReports.region, region));
        }

        // 1. Nombre de rapports
        const reportsCountResult = await db.select({ count: count() })
          .from(reforestationReports)
          .where(and(...filters));

        // 2. Total pépinières et gaines (Production)
        const productionStatsResult = await db.select({
          totalPep: sql<number>`sum(${reforestationProductionData.nbPepinieresPeriode})`,
          totalGaines: sql<number>`sum(${reforestationProductionData.gainesEnsemenceesPeriode})`
        })
        .from(reforestationProductionData)
        .innerJoin(reforestationReports, eq(reforestationProductionData.reportId, reforestationReports.id))
        .where(and(...filters));

        // 3. Total plants (Plants Data)
        const plantsStatsResult = await db.select({
          totalPlants: sql<number>`sum(${reforestationPlantsData.regieNbPlants} + ${reforestationPlantsData.priveIndivNbPlants} + ${reforestationPlantsData.villagCommNbPlants} + ${reforestationPlantsData.scolaireNbPlants})`
        })
        .from(reforestationPlantsData)
        .innerJoin(reforestationReports, eq(reforestationPlantsData.reportId, reforestationReports.id))
        .where(and(...filters));

        // 4. Total reboisement terrain (Field Data)
        const fieldStatsResult = await db.select({
          totalHa: sql<number>`sum(${reforestationFieldData.pmRegieHa} + ${reforestationFieldData.pmPriveIndivHa} + ${reforestationFieldData.pmVillagCommHa} + ${reforestationFieldData.pmScolaireHa})`,
          totalPlantsField: sql<number>`sum(${reforestationFieldData.pmRegiePlants} + ${reforestationFieldData.pmPriveIndivPlants} + ${reforestationFieldData.pmVillagCommPlants} + ${reforestationFieldData.pmScolairePlants})`
        })
        .from(reforestationFieldData)
        .innerJoin(reforestationReports, eq(reforestationFieldData.reportId, reforestationReports.id))
        .where(and(...filters));
        ...
        return {
          reportCount: Number(reportsCountResult[0]?.count || 0),
          nurseryCount: Number(productionStatsResult[0]?.totalPep || 0),
          gainesCount: Number(productionStatsResult[0]?.totalGaines || 0),
          totalPlants: Number(plantsStatsResult[0]?.totalPlants || 0),
          totalHa: Number(fieldStatsResult[0]?.totalHa || 0),
          totalPlantsField: Number(fieldStatsResult[0]?.totalPlantsField || 0)
        };
      } catch (error) {
        console.error("Erreur getRegionalReforestationStats:", error);
        return { reportCount: 0, nurseryCount: 0, totalPlants: 0, totalHa: 0 };
      }
    }

    async getMySectorAgentsByDomain(region: string, domain: string): Promise<User[]> {
      try {
        const results = await db.select({
          user: users
        })
        .from(users)
        .innerJoin(userDomains, eq(users.id, userDomains.userId))
        .where(and(
          eq(users.region, region),
          eq(userDomains.domain, domain),
          eq(userDomains.active, true)
        ));

        return results.map(r => r.user);
      } catch (error) {
        console.error("Erreur getMySectorAgentsByDomain:", error);
        return [];
      }
    }

    async getReforestationActivities(region?: string): Promise<any[]> {
      try {
        const filters = [eq(reforestationReports.status, 'valide')];
        if (region) {
          filters.push(eq(reforestationReports.region, region));
        ...
        const results = await db.select()
          .from(reforestationReports)
          .where(and(...filters))
          .orderBy(desc(reforestationReports.createdAt))
          .limit(10);

        return results;
      } catch (error) {
        console.error("Erreur getReforestationActivities:", error);
        return [];
      }
    }

    async checkReforestationReportExists(userId: number, period: string): Promise<boolean> {
      try {
        const results = await db.select({ id: reforestationReports.id })
          .from(reforestationReports)
          .where(and(
            eq(reforestationReports.createdBy, userId),
            eq(reforestationReports.period, period)
          ))
          .limit(1);
        return results.length > 0;
      } catch (error) {
        console.error("Erreur checkReforestationReportExists:", error);
        return false;
      }
    }

    async createReforestationReport(
      report: InsertReforestationReport,
      production: InsertReforestationProductionData[],
      plants: InsertReforestationPlantsData[],
      species: InsertReforestationSpeciesData[],
      field: InsertReforestationFieldData[]
    ): Promise<ReforestationReport> {
      try {
        return await db.transaction(async (tx) => {
          // 1. Créer le rapport principal
          const [newReport] = await tx.insert(reforestationReports).values(report).returning();
          const reportId = newReport.id;

          // 2. Insérer les données de production
          if (production.length > 0) {
            await tx.insert(reforestationProductionData).values(
              production.map(p => ({ ...p, reportId }))
            );
          }

          // 3. Insérer les données de plants
          if (plants.length > 0) {
            await tx.insert(reforestationPlantsData).values(
              plants.map(p => ({ ...p, reportId }))
            );
          }

          // 4. Insérer les données d'espèces
          if (species.length > 0) {
            await tx.insert(reforestationSpeciesData).values(
              species.map(s => ({ ...s, reportId }))
            );
          }

          // 5. Insérer les données de terrain
          if (field.length > 0) {
            await tx.insert(reforestationFieldData).values(
              field.map(f => ({ ...f, reportId }))
            );
          }
        ...
        console.error("Erreur createReforestationReport:", error);
        throw error;
      }
    }

    async getReforestationReports(filters: any): Promise<ReforestationReport[]> {
      try {
        const conditions = [];
        if (filters.region) conditions.push(eq(reforestationReports.region, filters.region));
        if (filters.departement) conditions.push(eq(reforestationReports.departement, filters.departement));
        if (filters.status) conditions.push(eq(reforestationReports.status, filters.status));
        if (filters.period) conditions.push(eq(reforestationReports.period, filters.period));
        if (filters.createdBy) conditions.push(eq(reforestationReports.createdBy, filters.createdBy));

        const reportCols = getTableColumns(reforestationReports);

        return await db.select({
          ...reportCols,
          creatorGrade: agents.grade,
          creatorFirstName: sql`COALESCE(${agents.prenom}, ${users.firstName})`,
          creatorLastName: sql`COALESCE(${agents.nom}, ${users.lastName})`,
          creatorMatricule: sql`COALESCE(${users.matricule}, ${agents.matriculeSol})`,
          creatorGenre: agents.genre,
        })
          .from(reforestationReports)
          .leftJoin(users, eq(users.id, reforestationReports.createdBy))
          .leftJoin(agents, eq(agents.userId, users.id))
          .where(conditions.length > 0 ? and(...conditions) : undefined)
          .orderBy(desc(reforestationReports.createdAt));
      } catch (error) {
        console.error("Erreur getReforestationReports:", error);
        return [];
      }
    }

    async getReforestationReportById(id: number): Promise<any> {
      try {
        const [report] = await db.select().from(reforestationReports).where(eq(reforestationReports.id, id)).limit(1);
        if (!report) return undefined;

        const production = await db.select().from(reforestationProductionData).where(eq(reforestationProductionData.reportId, id));
        const plants = await db.select().from(reforestationPlantsData).where(eq(reforestationPlantsData.reportId, id));
        const species = await db.select().from(reforestationSpeciesData).where(eq(reforestationSpeciesData.reportId, id));
        const field = await db.select().from(reforestationFieldData).where(eq(reforestationFieldData.reportId, id));
        ...
        console.error("Erreur getReforestationReportById:", error);
        return undefined;
      }
    }

    async getConsolidatedNationalReforestationData(period: string): Promise<any> {
      try {
        // Au niveau national, on consolide les rapports de niveau 'region' validés
        const reports = await db.select({ id: reforestationReports.id })
          .from(reforestationReports)
          .where(and(
            eq(reforestationReports.period, period),
            eq(reforestationReports.status, 'valide'),
            eq(reforestationReports.level, 'region')
          ));

        const reportIds = reports.map(r => r.id);
        if (reportIds.length === 0) return { production: [], plants: [], species: [], field: [] };

        const production = await db.select().from(reforestationProductionData).where(inArray(reforestationProductionData.reportId, reportIds));
        const plants = await db.select().from(reforestationPlantsData).where(inArray(reforestationPlantsData.reportId, reportIds));
        const species = await db.select().from(reforestationSpeciesData).where(inArray(reforestationSpeciesData.reportId, reportIds));
        const field = await db.select().from(reforestationFieldData).where(inArray(reforestationFieldData.reportId, reportIds));

        return { production, plants, species, field };
      } catch (error) {
        console.error("Erreur getConsolidatedNationalReforestationData:", error);
        return { production: [], plants: [], species: [], field: [] };
      }
    }

    async getConsolidatedReforestationData(region: string, period: string): Promise<any> {
      try {
        // Au niveau régional, on consolide les rapports de niveau 'departement' de cette région validés
        const reports = await db.select({ id: reforestationReports.id })
          .from(reforestationReports)
          .where(and(
            eq(reforestationReports.region, region),
            eq(reforestationReports.period, period),
            inArray(reforestationReports.status, ['soumis', 'valide']),
            inArray(reforestationReports.level, ['commune', 'arrondissement', 'departement'])
          ));
        ...
        console.error("Erreur getConsolidatedReforestationData:", error);
        return { production: [], plants: [], species: [], field: [] };
      }
    }
    ...
    async updateReforestationReportStatus(id: number, status: string): Promise<ReforestationReport | undefined> {
      try {
        ...
        console.error("Erreur updateReforestationReportStatus:", error);
        return undefined;
      }
    }

    async deleteReforestationReport(id: number): Promise<boolean> {
      try {
        await db.transaction(async (tx) => {
          await tx.delete(reforestationProductionData).where(eq(reforestationProductionData.reportId, id));
          await tx.delete(reforestationPlantsData).where(eq(reforestationPlantsData.reportId, id));
          await tx.delete(reforestationSpeciesData).where(eq(reforestationSpeciesData.reportId, id));
          await tx.delete(reforestationFieldData).where(eq(reforestationFieldData.reportId, id));
          await tx.delete(reforestationReports).where(eq(reforestationReports.id, id));
        });
        return true;
      } catch (error) {
        console.error("Erreur deleteReforestationReport:", error);
        return false;
      }
    }

    async getLastReforestationReport(location: { region: string; departement?: string; arrondissement?: string; commune?: string }): Promise<ReforestationReport | undefined> {
      try {
        const conditions = [eq(reforestationReports.region, location.region)];
        if (location.departement) conditions.push(eq(reforestationReports.departement, location.departement));
        if (location.arrondissement) conditions.push(eq(reforestationReports.arrondissement, location.arrondissement));
        if (location.commune) conditions.push(eq(reforestationReports.commune, location.commune));

        const results = await db.select()
          .from(reforestationReports)
          .where(and(...conditions))
          .orderBy(desc(reforestationReports.reportDate))
          .limit(1);

        return results[0];
      } catch (error) {
        console.error("Erreur getLastReforestationReport:", error);
        return undefined;
      }
    }

    async getReforestationLocalites(departement: string): Promise<ReforestationLocalite[]> {
      try {
        return await db.select()
          .from(reforestationLocalites)
          .where(and(
            eq(reforestationLocalites.departement, departement),
            sql`${reforestationLocalites.deletedAt} IS NULL`
          ));
      } catch (error) {
        console.error("Erreur getReforestationLocalites:", error);
        return [];
      }
    }

    async addReforestationLocalite(data: InsertReforestationLocalite): Promise<ReforestationLocalite> {
      try {
        const result = await db.insert(reforestationLocalites).values(data).returning();
        return result[0];
      } catch (error) {
        console.error("Erreur addReforestationLocalite:", error);
        throw error;
      }
    }
    ...
    async softDeleteReforestationLocalite(id: number, userId: number): Promise<boolean> {
      try {
        ...
        console.error("Erreur softDeleteReforestationLocalite:", error);
        return false;
      }
    }

    async getCatalogSpecies(): Promise<any[]> {
      try {
        return await db.select().from(reforestationSpeciesCatalog).orderBy(reforestationSpeciesCatalog.name);
      } catch (error) {
        console.error("Erreur getCatalogSpecies:", error);
        return [];
      }
    }

    async addCatalogSpecies(name: string, category: string): Promise<any> {
      try {
        const result = await db.insert(reforestationSpeciesCatalog).values({ name, category }).returning();
        return result[0];
      } catch (error) {
        console.error("Erreur addCatalogSpecies:", error);
        throw error;
      }
    }

    async bulkAddCatalogSpecies(items: any[]): Promise<number> {
      try {
        const result = await db.insert(reforestationSpeciesCatalog).values(items).returning();
        return result.length;
      } catch (error) {
        console.error("Erreur bulkAddCatalogSpecies:", error);
        return 0;
      }
    }

    async deleteCatalogSpecies(id: number): Promise<boolean> {
      try {
        const result = await db.delete(reforestationSpeciesCatalog).where(eq(reforestationSpeciesCatalog.id, id)).returning();
        return result.length > 0;
      } catch (error) {
        console.error("Erreur deleteCatalogSpecies:", error);
        return false;
      }
    }
    ...
    async getCatalogCategories(): Promise<any[]> {
      try {
        return await db.select().from(reforestationSpeciesCategories).orderBy(reforestationSpeciesCategories.name);
      } catch (error) {
        console.error("Erreur getCatalogCategories:", error);
        return [];
      }
    }

    async addCatalogCategory(name: string): Promise<any> {
      try {
        const result = await db.insert(reforestationSpeciesCategories).values({ name }).returning();
        return result[0];
      } catch (error) {
        console.error("Erreur addCatalogCategory:", error);
        throw error;
      }
    }

    async updateCatalogCategory(id: number, name: string): Promise<any> {
      try {
        const result = await db.update(reforestationSpeciesCategories)
          .set({ name })
          .where(eq(reforestationSpeciesCategories.id, id))
          .returning();
        return result[0];
      } catch (error) {
        console.error("Erreur updateCatalogCategory:", error);
        return undefined;
      }
    }
    ...
    async deleteCatalogCategory(id: number): Promise<boolean> {
      try {
        const result = await db.delete(reforestationSpeciesCategories).where(eq(reforestationSpeciesCategories.id, id)).returning();
        return result.length > 0;
      } catch (error) {
        console.error("Erreur deleteCatalogCategory:", error);
        return false;
      }
    }
