...
import { eq, and } from 'drizzle-orm'; // Assure-toi que ce chemin vers ton instance Prisma est correct
...
export const getMySectorAgents = async (req: Request, res: Response) => {
  try {
    ...
    const queryConditions = [eq(users.role, 'sub-agent')];
    if (regionalAgentRegion) {
      queryConditions.push(eq(users.region, regionalAgentRegion));
    } else {
    ...
    const sectorAgents = await prisma
      .select()
      .from(users)
