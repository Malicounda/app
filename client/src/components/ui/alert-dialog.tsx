  ...
  <AlertDialogPrimitive.Root
    ...
    onOpenChange={(open) => {
      if (!open && document.body.hasAttribute("data-session-locked")) {
        // Session verrouillée : empêcher la fermeture du dialog pour préserver les données
        return;
      }
      if (!open) {
        scheduleRestore();
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    ...
    className={cn(
      "fixed inset-0 z-[10020] bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:pointer-events-none",
      className
    )}
    {...props}
    ref={ref}
